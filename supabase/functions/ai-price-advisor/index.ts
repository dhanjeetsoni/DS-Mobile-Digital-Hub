...
// ---------------------------------------------------------------------------
// ai-price-advisor — Supabase Edge Function (Phase 6: "AI-based selling
// price/MRP suggestion when adding a product").
//
// Standalone function (not a route inside ai-gateway) matching the contract
// src/services/phase6.ts's suggestForProductPrice() already expected —
// `supabase.functions.invoke("ai-price-advisor", { body: input })`.
//
// Deliberately does NOT claim to be a live-market-price lookup: this Gemini
// call has no web-search grounding wired in, so it reasons from general
// knowledge of Indian mobile/accessory retail plus whatever purchase price
// and existing prices the shop gives it — never fabricates a "source" URL
// (the `sources` field in the response is always empty for that reason;
// PriceSuggestion's `sources` field exists in the client type for a
// possible future grounded-search version, not used yet). Confidence is
// explicitly downgraded when no purchase price is given, since a price
// suggestion with no cost basis at all is much closer to a guess.
//
// Same multi-key failover/rate-limit/auth pattern as ai-gateway/index.ts —
// duplicated rather than imported because Supabase Edge Functions don't
// share code across function boundaries in this project (every other
// function here, e.g. daily-digest-worker, telegram-connect, does the
// same). Includes the 2026-09-07 network-transport-error retry fix from
// the start (see ai-gateway/index.ts's classifyGeminiFailure for the full
// writeup of why that fix exists).
// ---------------------------------------------------------------------------
...
// --- Multi-key Gemini failover pool (see ai-gateway/index.ts for the full
// original writeup; kept in sync here) ---
...
function classifyGeminiFailure(err: any): "quota" | "invalid" | "unavailable" | null {
  ...
  // 2026-09-07: genuine network-transport failures — see the full writeup
  // in ai-gateway/index.ts's classifyGeminiFailure for why this matters.
...
function normalizeRecommendation(raw: any, input: PriceAdvisorInput) {
  ...
  // Confidence is explicitly downgraded server-side (not just left to the
  // model's own judgement) when there's no purchase price to reason from —
  // a suggestion with zero cost basis is a much rougher guess, and this
  // shouldn't depend on the model remembering to say so itself.
  ...
  return {
    ...
    // Never web-grounded (no search tool wired into this call) — always
    // empty rather than let the model invent a plausible-looking citation.
      ...
      config: {
        ...
        responseSchema: {
          ...
          properties: {
            ...
            recommendedSellingPrice: { type: Type.NUMBER },
...
Deno.serve(async (req: Request) => {
  ...
  if (!checkRateLimit(`price-advisor:${clientIp(req)}`, 60_000, 15)) {
  ...
  if (!input.productName.trim() || !input.category.trim()) {
    return json({ success: false, error: "Product name aur category chahiye." }, 400);
  }
