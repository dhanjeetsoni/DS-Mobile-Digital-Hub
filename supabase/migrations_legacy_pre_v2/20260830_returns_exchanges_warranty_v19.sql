...
-- follow-up (v20): status updates keyed by claim_no instead of local id,
-- since the local warranty claim id never reconciles with the server row's
-- uuid (same reasoning as returns/exchanges using idempotency_key/return_no
-- instead of matching ids). Applied as a separate migration file below.
