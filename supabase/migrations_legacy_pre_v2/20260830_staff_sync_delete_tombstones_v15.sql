...
--
-- Root cause: save_store_state_for_user() (v11/v12) unions the server's
-- current purchases/suppliers/supplierPayments/secondHandKYCs/imeiRegistry/
...
-- future write path -- without needing to touch save_store_state(), whose
-- source is not part of this bundle). save_store_state_for_user() then
-- filters its merged output against the ledger so a tombstoned id can never
-- come back from a stale staff device. If a record with the same id
-- legitimately reappears in the store's true state (owner recreates it),
-- its tombstone is cleared automatically.
...
-- below (owned by the migration role, not by end users) ever touch this
-- table, so no RLS policy is added and all client roles are revoked.
...
-- save_store_state_for_user(): identical body to the v12 (lender_owner_projection)
