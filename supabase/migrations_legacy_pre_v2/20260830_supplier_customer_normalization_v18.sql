-- Two normalized entities, kept intentionally simple and ADDITIVE:
--
-- 1) suppliers — a real table finally backing supplier_transactions.supplier_id
--    (that column has existed with no FK target since no suppliers table
--    existed yet). App's local db.suppliers[]/db.supplierPayments[] JSON
--    stays the source of truth for the UI (SupplierKhataView already works
--    fully offline-first); this table is the cloud mirror/audit trail so
--    supplier data survives a lost/reinstalled device and can be reported on
--    with SQL later, same relationship the sales/sale_items tables have to
--    the old JSON blob.
--
-- 2) customers + customer_payments — same idea for Customer Khata (udhaar).
--    There was previously no normalized payments table at all for
--    khata collections (only sale-level due tracking) — this migration adds
--    ability to log an individual khata payment against a customer.
--
-- Neither table recomputes balances server-side; the client remains
-- authoritative for totalPayable/totalDue (same trust model as the rest of
-- this app's offline-first design) and simply mirrors each write here.

create table if not exists public.suppliers (
  id uuid primary key default gen_random_uuid(),
  store_id uuid not null references public.stores(id) on delete cascade,
  client_id text, -- the local "sup_xxx" id from db.suppliers[], for idempotent upsert-by-name matching
  name text not null,
  phone text,
  category text,
  address text,
...
alter table public.supplier_transactions
  ...
  not valid; -- NOT VALID: table is empty (0 rows) so this is instant; avoids locking if it weren't
...
-- Find-or-create by name (case-insensitive) within the store, so repeated
-- calls with the same supplier name never create duplicates. Updates the
-- contact fields when new values are supplied. SECURITY DEFINER so it can
-- run as part of an offline-queue replay under the caller's own store scope
-- (checked explicitly against my_store_id(), same guard as other RPCs here).
...
-- Logs a payment made TO a supplier (audit trail — client keeps computing
-- the running totalPayable itself). Idempotent via the same idempotency_key
-- convention used by sync_queue.operation_id.
...
-- Find-or-create by phone within the store (phone is the natural dedupe key
-- for a shop's customer list — see customers_store_phone_uidx above).
...
-- Logs a khata payment collected FROM a customer. Idempotent the same way
-- as record_supplier_payment above.
